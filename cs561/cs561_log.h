...
struct FileInfo{
    ...
    FileInfo(const FileMetaData* file, uint64_t overlapping_bytes_=0) : 
...
struct CompactionInfo{
    ...
    CompactionInfo(int level_, std::vector<FileMetaData*>* level_file, const std::vector<uint64_t>& file_overlapping_ratio_, int num_level, size_t hash_value_, size_t index_) : 
        ...
        level_files_info = std::vector<std::vector<FileInfo>>(num_level);
        for(int i=0; i<num_level; ++i){
            level_files_info[i].reserve(level_file[i].size());
            for(size_t j=0; j<level_file[i].size(); ++j){
                if(level == i){
                    level_files_info[i].emplace_back(FileInfo(level_file[i][j], file_overlapping_ratio_[j]));
                } else {
                    level_files_info[i].emplace_back(FileInfo(level_file[i][j]));
                }
            }
        }
    }
...
private:
     ...
     * @param compactions_info a vector records compaction informatio of every compaction
    */
    static void LogCompactionsInfo(std::ofstream& target, const std::vector<CompactionInfo>& compactions_info);
    ...
    */
    static void LogCompactionInfo(std::ofstream& target, const CompactionInfo& compaction_info, int compaction_index);
    ...
    */
    static void LogCompactionInfoForLevel(std::ofstream& target, const CompactionInfo& compaction_info, int level);
...
public:
    ...
    */
    static void LogMinimum(size_t WA, size_t left_bytes, const std::vector<CompactionInfo>& compactions_info);
    ...
    */
     ...
     * @param temp a vector of Fsize which contains the filemetadata of a version
     * @param hash_value hash value of this version
    */
    static void LogVersion(const std::vector<Fsize>& temp, size_t hash_value);

    /**
     * Log the compaction information for the run with the minimum write amplification
     * @param compactions_info a vector records compaction informatio of every compaction
    */
    static void LogMinimumCompactionInfo(const std::vector<CompactionInfo>& compactions_info);
    ...
    */
    static void LogRegularCompactionInfo(const CompactionInfo& compaction_info, int compaction_index);
    ...
    */
    static void AlignOutput(std::ofstream& of, const std::string& align_target, const std::string& output);
    
    /**
     * Read the fully enumerated sign of the first version of a level
     * @param level level
    */
    static std::string ReadFinishEnumeration(int level);

    /**
     * Log the write amplification of current run if the current is the optimal one or is the baseline
